...
const StandardTokenMock = artifacts.require('../../mocks/StandardTokenMock');
...
contract('StandardToken', function ([_, owner, recipient, anotherAccount ]) {
  ...
  beforeEach(async function () {
    this.whitelist = await TokenWhitelist.new()
    this.token = await StandardTokenMock.new(owner, 100, this.whitelist.address);
    // whitelist all addresses in this test suite
    await this.whitelist.enableWallet(owner)
    await this.whitelist.enableWallet(recipient)
    await this.whitelist.enableWallet(anotherAccount)
  });
  ...
  describe('approve', function () {
    ...
    /** This feature was disabled as all addresses must be whitelisted */
    describe.skip('when the spender is the zero address', function () {
      const amount = 100;
